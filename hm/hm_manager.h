...
namespace leveldb{
    ...
    class HMManager {
    public:
        HMManager(const Comparator *icmp, int reset);
        ~HMManager();
        ...
        // double space_amplification(int num, int key_size, int value_size);
        // get the number of zones which have full, open or closed state
        uint32_t get_zonenum_sa(uint32_t *zone_num, uint32_t *space_amp);
    ...
    private:
        ...
        // tracking valid data of each zone
        uint64_t *zone_valid_data_; // init in constructor
