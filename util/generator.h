...
static double ThreadLocalRandomDouble(double min = 0.0, double max = 1.0) {
...
class ZipfianGenerator : public Generator<uint64_t> {
 public:
  ...
  // ZipfianGenerator(const ZipfianGenerator &gen) = default;
...
static uint32_t ThreadLocalRandomInt() {
  static thread_local std::random_device rd;
  static thread_local std::minstd_rand rn(rd());
  return rn();
}
