...
class HMWritableFileL0 : public WritableFile {    //hm write L0 level file
  ...
  public:
    ...
    virtual Status Append(const Slice &data) {
      // fprintf(stdout, "HMWritableFileL0 Append %s %lu\n", fname_.c_str(), data.size());
    ...
    virtual Status Close() {
      // fprintf(stdout, "HMWritableFileL0 Close %s\n", fname_.c_str());
      return Status::OK();
    }

    virtual Status Flush() {
      // fprintf(stdout, "HMWritableFileL0 Flush %s\n", fname_.c_str());
      return Status::OK();
    }

    virtual Status Sync() {
      // fprintf(stdout, "HMWritableFileL0 Sync %s %lu\n", fname_.c_str());

      ssize_t ret = hm_manager_->hm_write(level_,Parsefname(fname_), buf_, total_size_);
      if(ret > 0){
          return Status::OK();
      }
      return Status::IOError("L0 sync error!");
    }
...
class PosixEnv : public Env {
 public:
  ...
  virtual Status NewWritableFile(const std::string& fname,
                                 WritableFile** result,
                                 int level = -1) {
    // fprintf(stdout, "NewWritableFile %s\n", fname.c_str());
    Status s;
    if(isSSTableName(fname)){
        if(level == 0){
